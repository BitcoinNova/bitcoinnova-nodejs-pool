...
if(config.InfiniumAlgo){
		 if(config.InfiniumAlgo=="cryptonight") infinium_algo = 0;
		 if(config.InfiniumAlgo=="cryptonight-zls") infinium_algo = 1;
		 if(config.InfiniumAlgo=="cryptonight-lite") infinium_algo = 2;}
...
let templateData = JSON.stringify({
	...
	params: {
		...
		wallet_address: config.poolServer.poolAddress, mining_algo: infinium_algo
	}
})
