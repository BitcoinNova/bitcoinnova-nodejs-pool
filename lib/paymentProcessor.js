...
if (config.InfiniumAlgo === "cryptonight" || config.InfiniumAlgo === "cryptonight-lite"|| config.InfiniumAlgo === "cryptonight-zls") {
...
function runInterval () {
	async.waterfall([
		...
		function (balances, minPayoutLevel, callback) {
			...
			callback(true,transferCommands);
		}
	...
	async.waterfall([
		...
		function (balances, minPayoutLevel, callback) {
			...
			for (let worker in payments) {
				...
				config.payments.feePerByte = config.payments.feePerByte || false
				...
				if (!transferCommands[commandIndex] && config.payments.feePerByte === true) {
					transferCommands[commandIndex] = {
						redis: [],
						amount: 0,
						rpc: {
			...
			async.filter(transferCommands, function (transferCmd, cback) {
				...
				if (daemonType === "bytecoin" && config.payments.feePerByte === true) {
					rpcCommand = "sendTransaction";
					rpcRequest = {
						transfers: transferCmd.rpc.destinations,
						anonymity: ringSize,
				...
				apiInterfaces.rpcWallet(rpcCommand, rpcRequest, function (error, result) {
					...
					if (config.payments.feePerByte === false) {
					transferCmd.redis.push(['zadd', config.coin + ':payments:all', now, [
						txHash,
						transferCmd.amount,
						transferCmd.rpc.fee,
					...
					if (config.payments.feePerByte === true) {
					transferCmd.redis.push(['zadd', config.coin + ':payments:all', now, [
						txHash,
						transferCmd.amount,
						result.fee,
				...
				let paymentSucceeded = succeeded === true ? 1 : 0;
				let failedAmount = transferCommands.length - paymentSucceeded;
				...
				log('info', logSystem, 'Payments splintered and %j successfully sent, %j failed', [paymentSucceeded, failedAmount]);
