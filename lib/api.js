	...
	// Else fallback to getlastblockheader
		...
		apiInterfaces.rpcDaemon('getlastblockheader', {}, function (error, reply) {
			if (error) {
				...
				let algo_diff = 0;
				if(config.InfiniumAlgo){
						if(config.InfiniumAlgo==="cryptonight") algo_diff = blockHeader.difficulty;
						if(config.InfiniumAlgo==="cryptonight-zls") algo_diff = blockHeader.second_difficulty;
						if(config.InfiniumAlgo==="cryptonight-lite") algo_diff = blockHeader.third_difficulty;
				}else{
					algo_diff = blockHeader.difficulty;
				}

				callback(null, {
					difficulty: blockHeader.difficulty,
					difficulty: algo_diff,
...
function getLastBlockData (callback) {
	apiInterfaces.rpcDaemon('getlastblockheader', {}, function (error, reply) {
		...
		callback(null, {
			...
			height: blockHeader.height,
			timestamp: blockHeader.timestamp,
			reward: blockHeader.reward  - ((config.blockUnlocker.fixedFee * config.coinUnits) || 0),
			hash: blockHeader.hash
		});
	});
}
