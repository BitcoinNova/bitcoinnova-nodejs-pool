...
/**
 * Offset reduction for infinitum-8 coin
 **/

 
config.offsetInfinityReducer === config.offsetInfinityReducer || false;

if (config.offsetInfinityReducer === true) {
	if (config.daemonType === 'bytecoin') {
		previousOffset = config.previousOffset || 3;
		offset = config.offset || 2;
	}
}
...
Miner.prototype = {
	...
	getJob: function () {
		...
		if (mergedMining) {
			if (this.lastBlockHeight === blockTemplate.height && (!currentChildBlockTemplate[this.activeChildPool] || this.lastChildBlockHeight === currentChildBlockTemplate[this.activeChildPool].height) && !this.pendingDifficulty && this.cachedJob !== null && !config.daemon.alwaysPoll) {
