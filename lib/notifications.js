...
notificationSettings.discordMessage['test'] = "This is a test notification from the pool.";
...
exports.sendToAll = function (id, variables) {
	...
	// Send to all pool discord addresses
	sendToDiscordChannel(id, variables);
}
...
function sendToTelegramChannel(id, variables) {
	...
	if (config.telegram && config.telegram.enabled) {
		...
		let chatId = '@' + channel;
		telegram.sendMessage(chatId, config.coin + ': ' + message);
	}
}
 ...
 **/
 function sendToDiscordChannel(id, variables) {
	// Set custom variables
	variables = setCustomVariables(variables);
	// Send notification
	if (config.discord && config.discord.enabled) {
		...
		try {
			client.once('ready', () => {
				log('info', logSystem, 'Discord client is ready');
				let message = getDiscordMessage(id, variables);
				if (!message || message === '') {
					log('info', logSystem, 'Notification disabled for %s: empty discord message.', [id]);
					return;
				}

				if (!channel) {
					log('error', logSystem, 'No discord channel specified in configuration!');
					return;
				}
				client.channels.cache.get(channel).send(config.coin + ': ' +message);
			});
			client.login(token);
		} catch (error) {
			log('error', logSystem, 'Discord error: ', [error]);
			return;
		}
...
exports.sendToDiscordChannel = sendToDiscordChannel;
...
// Get telegram message
function getDiscordMessage(id, variables) {
	let discordVars = {};
	if (discordVars) {
		for (let varName in variables) {
			let value = variables[varName].toString();
			value = value.replace(/\*/g, '.');
			value = value.replace(/_/g, ' ');
			discordVars[varName] = value;
		}
	}
	let message = replaceVariables(notificationSettings.discordMessage[id], discordVars) || '';
	return message;
}
/**
